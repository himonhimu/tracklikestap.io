...
export const api = {
  ...
  updateUser: (id, body) =>
    request(`/api/auth/users/${id}`, { method: 'PUT', body: JSON.stringify(body) }),
  getTotalUsers: (urlContains, userId) =>
    request('/api/analytics/users/total' + buildQuery({ url_contains: urlContains, user_id: userId })),
  getUsersByDevice: (urlContains, userId) =>
    request('/api/analytics/users/by-device' + buildQuery({ url_contains: urlContains, user_id: userId })),
  getUsersByLocation: (urlContains, userId) =>
    request('/api/analytics/users/by-location' + buildQuery({ url_contains: urlContains, user_id: userId })),
  getRecentUsers: (limit = 50, urlContains, userId) =>
    request('/api/analytics/users/recent' + buildQuery({ limit, url_contains: urlContains, user_id: userId })),
  getEventCounts: (urlContains, userId) =>
    request('/api/analytics/events/counts' + buildQuery({ url_contains: urlContains, user_id: userId })),
  getPurchases: (limit = 50, urlContains, userId) =>
    request('/api/analytics/events/purchases' + buildQuery({ limit, url_contains: urlContains, user_id: userId })),
  getAddToCart: (limit = 50, urlContains, userId) =>
    request('/api/analytics/events/add-to-cart' + buildQuery({ limit, url_contains: urlContains, user_id: userId })),
  ...
  getEventsByTime: (eventType, granularity = 'daily', urlContains, days, dateFrom, dateTo, userId) =>
    request(
